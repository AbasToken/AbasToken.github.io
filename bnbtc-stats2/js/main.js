...
const _HASHRATE_MULTIPLIER = 2 ** 32; /* TODO: calculate this from max_target (https://en.bitcoin.it/wiki/Difficulty) */
...
var eth = new Eth(new Eth.HttpProvider("https://bsc-dataseed1.binance.org"));
var etharchive = new Eth(new Eth.HttpProvider("https://bsc-dataseed1.binance.org"));
...
var known_miners = {
    "0x1ae8537afe4e8fc9fde383f955c0200e8c16e707": ["0xPool", "https://0xpool.me", pool_colors.blue],
    "0x98b155d9a42791ce475acc336ae348a72b2e8714": ["0xBTCpool", "http://0xBTCpool.com", pool_colors.orange],
...
function calculateNewMiningDifficulty(current_difficulty,
    ...
    var target_eth_blocks_since_last_difficulty_period = epochs_mined.mul(new Eth.BN(200));
...
stats = [
    ...
    ['Price Per Token', null, "$", 1, null],
    ['Circulating Supply Marketcap', null, "$", 1, null],
    ['Liquidity', null, "$", 1, null],
    ['One Day Volume', null, "$", 1, null],
    ...
    ['Last Bsc Reward Block', token.lastRewardBscBlockNumber, "", 1, null], /* mining */
    ['Last Bsc Block', eth.blockNumber, "", 1, null], /* mining */
    ...
    /*['', null, "", 1, null], /* */
    ['Token Holders', null, "holders", 1, null],
...
function updateStatsThatHaveDependencies(stats) {
    ...
    current_eth_block = getValueFromStats('Last Bsc Block', stats)
  ...
  el_safe('#InflationPercentageperYear').innerHTML = "<b>" + ((100 * ( amt1+amt2)) / (current_supply + amt1 + amt2)).toLocaleString(undefined, {
    ...
    $.getJSON('https://forgetoken.org/api/bnbitcoin/bnbtc-stats',
        function (data) {
            setValueInStats('Price Per Token', data["price"], stats);
		});
   
console.log("Price: ", getValueFromStats('Price Per Token', stats))


		console.log("AFRICA: ", getValueFromStats('Tokens Minted', stats))

 var valzzz = getValueFromStats('Price Per Token', stats) * getValueFromStats('Tokens Minted', stats)

    setValueInStats('Circulating Supply Marketcap', valzzz, stats);
    el_safe('#CirculatingSupplyMarketcap').innerHTML = "<b>" + Math.round(valzzz).toLocaleString() + "</b> $";
...
function updateThirdPartyAPIs() {
    ...
    $.getJSON('https://forgetoken.org/api/bnbitcoin/bnbtc-stats',
        function (data) {
            el('#TokenHolders').innerHTML = "<b>" + data["holdersCount"].toLocaleString() + "</b> holders";
            el('#TokenTransfers').innerHTML = "<b>" + data["transactions"].toLocaleString() + "</b> transfers";
	    el('#PricePerToken').innerHTML = "<b>" + data["price"] + "</b> $";
	    setValueInStats('Price Per Token', data["price"], stats);
   
            el('#OneDayVolume').innerHTML = "<b>" + Math.round(data["volume"]).toLocaleString() + "</b> $";
            el('#Liquidity').innerHTML = "<b>" + Math.round(data["liquidity"]).toLocaleString() + "</b> $";
...
async function updateAllMinerInfo(eth, stats, hours_into_past) {
    ...
    var last_reward_eth_block = getValueFromStats('Last Bsc Reward Block', stats)
    var current_eth_block = getValueFromStats('Last Bsc Block', stats)
    ...
    const serverUrl = "https://moralis.0xpool.me/server";
    const appId = "COTZf2oHkYGE1ZbgahQCPpV3jK745JJKgQOA8Rms";
    Moralis.start({serverUrl: serverUrl, appId: appId});

    const bnbtcmints = Moralis.Object.extend("BnbtcmintLogs");
    const query = new Moralis.Query(bnbtcmints);
    query.greaterThanOrEqualTo('blockNumber', last_difficulty_start_block)
    query.select('transactionHash', 'blockNumber', 'from')

    query.limit(2500);

    const mints = await query.find();

    let result = mints.map(item => {
        return item.attributes;
    })

    var start_log_search_at = Math.max(last_difficulty_start_block, last_imported_mint_block + 1)
    log('start log search at', start_log_search_at);
    ...
    // var iterations = Math.ceil((blocks_to_search / 5000));
    // if (iterations <= 0) {
    //     iterations = 1
    // }
    ...
    //     log('run', run + 1);
    //     start_log_search_at_loop = start_log_search_at + (run * 5000)
    //     run++;
    //     stop_log_search_at_loop = start_log_search_at_loop + 4999
    ...
    result.forEach(function (transaction) {
        ...
        var block_number = parseInt(transaction['blockNumber']);
        //var miner_address = getMinerAddressFromTopic(transaction['topics'][1].toString());
        var miner_address = transaction['from']
...
// })
    ...
    last_diff_block_storage = Number(localStorage.getItem('lastDifficultyStartBlock'));
    last_imported_mint_block = Number(localStorage.getItem('lastMintBlock'));
    mint_data = localStorage.getItem('mintData');
    ...
    /* descending */
    sorted_miner_block_count.sort((a, b) => {
        return b[1] - a[1];
    });
...
// $(window).trigger('resize');
    ...
    /* fill in block info */
    var dt = new Date();
    var innerhtml_buffer = '<tr><th>Time (Approx)</th><th>Bsc Block #</th>'
...
function updateStatsTable(stats) {
    stats.forEach(function (stat) {
        ...
        if (stat_function !== null) {
            stat_function().then(set_value(stats, stat_name, stat_unit, stat_multiplier, (value) => {
                stat[4] = value
            }));
        }
    });

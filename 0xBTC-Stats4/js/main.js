...
const _MINT_TOPIC = "0x8c5be1e5ebec7d5bd14f71427d1e84f3dd0314c0f7b2291e5b200ac8c7c3b925";
...
function updateAllMinerInfo(eth, stats, hours_into_past){
  ...
  var last_difficulty_start_block = getValueFromStats('Last Difficulty Start Block', stats) - 5000000
  ...
  try {
    var last_diff_block_storage = Number(localStorage.getItem('lastDifficultyStartBlock_0xbtc22'));
    last_imported_mint_block = Number(localStorage.getItem('lastMintBlock_0xbtc22'));
    var mint_data = localStorage.getItem('mintData_0xbtc22');
  ...
  .then((result) => {
	...
	console.log("YES IT IS3");
      mined_blocks.unshift([block_number, tx_hash, miner_address, miner_address_TO])
    ...
    if (result.length > 0) {
      localStorage.setItem('mintData_0xbtc22', JSON.stringify(mined_blocks));
      localStorage.setItem('lastMintBlock_0xbtc22', mined_blocks[0][0]);
      localStorage.setItem('lastDifficultyStartBlock_0xbtc22', last_difficulty_start_block.toString());
    }
    ...
    mined_blocks.forEach(function(block_info) {
      ...
      var addrTO = block_info[3];
      var miner_name_link = getMinerNameLinkHTML(addr, known_miners);

      var transaction_url = _BLOCK_EXPLORER_TX_URL + tx_hash;
      var block_url = _BLOCK_EXPLORER_BLOCK_URL + eth_block;

	console.log("print miner name: ", addr, " prinit known_miners2[addr], ",String(known_miners2[addr]));
      //log('hexcolor:', hexcolor, address_url);
	console.log("MINER TOO: ", addrTO);
	//must be on list known_miners but not on known_miners2
	if((known_miners[addr] !== undefined) && !(known_miners2[addrTO] !== undefined )){
...
function getMinerInfoCSV(eth, stats, hours_into_past){
  ...
  /* and https://ethereum.stackexchange.com/questions/12950/what-are-event-topics/12951#12951 */
  eth.getLogs({
    fromBlock: current_eth_block - num_eth_blocks_to_search,
    toBlock: current_eth_block,
    address: _CONTRACT_ADDRESS,
    topics: [_MINT_TOPIC, null],
  })
  .then((result) => {
    ...
    // Start file download.
    downloadTextAsFile("0xbtc-blocks-_0xbtc22" + date_now.toLocaleTimeString() + ".csv",
                       csv_text);

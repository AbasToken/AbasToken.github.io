...
function generateHashrateAndBlocktimeGraph(eth, target_cv_obj, era_cv_obj, tokens_minted_cv_obj) {
    ...
    function getHashrateDataFromDifficultyAndErasPerBlockData(difficulty_data, eras_per_block_data) {
        var expected_eras_per_block = 1 / 200; /* should be 40 times slower than ethereum (with 15-second eth blocks) */
    ...
    var difficulty_data = convertValuesToChartData(target_values,
        (x) => {
            return _MAXIMUM_TARGET_BN.div(x)
        });
    ...
    for (var i = 0; i < eras_per_block_data.length; i += 1) {
        ...
        average_reward_time_data.push({
            ...
            /* 1 / (eras per block * 20 bsc blocks per minute) */
            y: 1 / (eras_per_block_data[i].y * 20),
        })
    }
    ...
    if (hashrate_based_on_difficulty > max_hashrate_value) {
        ...
        max_difficulty_value = max_difficulty_value * 1.5
    }
    log('max_hashrate_value', max_hashrate_value);
    log('max_difficulty_value', max_difficulty_value);
            ...
            tooltips: {
                callbacks: {
                    label: function (tooltipItem, data) {
                        ...
                        label += " @ Bsc block #" + tooltipItem.xLabel;
...
async function updateHashrateAndBlocktimeGraph(eth, start_eth_block, end_eth_block, num_search_points) {
    ...
    */
    ...
    //       enough.
    var last_diff_start_blocks = new contractValueOverTime(etharchive, _CONTRACT_ADDRESS, _LAST_DIFF_START_BLOCK_INDEX, 'diffStartBlocks');
    // 'reward era' is at location 7
    var era_values = new contractValueOverTime(etharchive, _CONTRACT_ADDRESS, _ERA_INDEX, 'eraValues');
    // 'tokens minted' is at location 20
    var tokens_minted_values = new contractValueOverTime(etharchive, _CONTRACT_ADDRESS, _TOKENS_MINTED_INDEX, 'tokensMinted');
    // 'mining target' is at location 11
    var mining_target_values = new contractValueOverTime(etharchive, _CONTRACT_ADDRESS, _MINING_TARGET_INDEX, 'miningTargets');

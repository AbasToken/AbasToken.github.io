...
/* intrinsic values */
/* intrinsic values */
var _SECONDS_PER_ETH_BLOCK = 0.35;
...
const _CONTRACT_ADDRESS = "0x0B549125fbEA37E52Ee05FA388a3A0a7Df792Fa7"; //ABAS
...
const _ERA_INDEX = '8'; //getEpoch
const _TOKENS_MINTED_INDEX = '23';
const _MINING_TARGET_INDEX = '13';
...
var eth = new Eth(new Eth.HttpProvider("https://arbitrum-mainnet.infura.io/v3/5127ae1e5ebc4c999274f109eca22dd4"));
...
stats = [
 ...
 ['Pool 2 Num1 Token',                token11.getReserves,                      "ABAS",                 0.000000000000000001,          null     ], /* mining */
    ['Pool 2 Num2 Token',                token11.getReserves,                      "ETH",                 0.000001,          null     ], /* mining */
 ['USD Value of 1 ABAS',                              null,                                 "",                 1,          null     ], /* */
 ['Circulating Supply Market Cap',                              null,                                 "",                 1,          null     ], /* */
 	 ['SushiSwap 1% Liquidty Pool for ABAS - ETH',                              null,                                 "",                 1,          null     ], /* */
 ['Total Supply in all Sushiswap Pool',               token4.totalSupply,                      "",                0.000000000000000001,          null     ], /* mining */
   ['Total Supply Staking Contract',   token3.totalSupply,  "",                 0.000000000000000001,          null     ], /* mining difficulty */
  ...
  ['Pool 1 Num1 Token',                token10.getReserves,                      "ABAS",                 0.000000000000000001,          null     ], /* mining */
    ['Pool 1 Num2 Token',                token10.getReserves,                      "ETH",                 0.000000000000000001,          null     ], /* mining */
     ['APY of Pool',                token4.totalSupply,                      "ETH",                 0.000000000000000001,          null     ], /* mining */
        ['Total Liquidity',               null,                      "",                 1,          null     ], /* mining */
       ['Percent Of Liquidity In Staking',               null,                      "",                 1,          null     ], /* mining */
 ['',                              null,                                 "",                 1,          null     ], /* */
...
['Mining Difficulty',             token.getMiningDifficulty,            "",                 1,          null     ], /* mining difficulty */
 ...
 ['Average Reward per Solve',   null,                                 "minutes",          1,          null     ], /* mining difficulty */
  ...
  ['Slow Mints Until NFT mint is active',    token.mintNFTGOBlocksUntil,                                 " Blocks of at least 12 minutes+",                 1,          null     ], /* mining blocks per adjustment MUST KEEP MUST KEEP MUST */
...
['Tokens distributed via Mining',                 token.tokensMinted,                   _CONTRACT_SYMBOL,   0.000000000000000001, null     ], /* supply */
 ...
 ['Tokens distributed via Auctions',                 token2.currentDay,                   _CONTRACT_SYMBOL,  32768, null     ], /* supply */
  ...
  ['Total ABAS Circulating Supply',     null,                                 "ABAS",          1,          null     ], /* mining difficulty */
...
['Max Supply for Current Era',    token.maxSupplyForEra,                _CONTRACT_SYMBOL,   0.00000000000000000245, null     ], /* mining */ 
...
['',                              null,                                 "",                 1,          null     ], /* */
 ...
 ['Total Ethereum spent at Auctions',  token2.totalBurnt,                   "E",   0.000000000000000001, null     ], /* supply */
  ['',                              null,                                 "",                 1,          null     ], /* */
...
//DO MY STUFF
  ...
  liqForge =2
  totalCircLPTokens = getValueFromStats('APY of Pool', stats)
  totalContractLPTokens = getValueFromStats('Total Supply Staking Contract', stats)
...
//Times the ABAS in the contract * contractBal / totalBal
 ...
 el_safe('#TotalLiquidity').innerHTML = "<b>" + (totalPool1Num1).toLocaleString('en-US', {minimumFractionDigits: 0, maximumFractionDigits: 3}) + " </b> ABAS  and " + "<b>" + (totalPool1Num2).toLocaleString('en-US', {minimumFractionDigits: 0, maximumFractionDigits: 6}) + " </b> ETH  ";
  el_safe('#PercentOfLiquidityInStaking').innerHTML = "<b>" + (ratioStakedPool1*100).toLocaleString('en-US', {minimumFractionDigits: 0, maximumFractionDigits: 6}) + " </b> % ";
...
// ffff / dddd = equation.  Get ffff now
  ...
  rewardRate1p1 = getValueFromStats('RewardRate Pool 1', stats)	
	ffff = rewardRate1p1 * (60*60*24*365)

  rewardRate3p1 = getValueFromStats('RewardRate3 Pool 1', stats)
	ffff2 = rewardRate3p1 * (60*60*24*365)


	console.log("totalrewardrateffff: ", ffff);
	console.log("totalrewardratedddd: ", dddd);
  ...
  el_safe('#APYofPool').innerHTML = "<b>" + (totalAPY).toLocaleString('en-US', {minimumFractionDigits: 0, maximumFractionDigits: 0}) + " </b> % APY  ";


  totalPool2Num1 = getValueFromStats('Pool 2 Num1 Token', stats)	

 totalPool2Num2 = getValueFromStats('Pool 2 Num2 Token', stats)	

ETH_PRICE = totalPool2Num2 / totalPool2Num1
console.log("ETH PRICE: ", ETH_PRICE);
ABAS_ETH_PRICE = totalPool1Num2 / totalPool1Num1
console.log("ABAS ETH PRICE: ", ETH_PRICE);
ABAS_USD_PRICE = ETH_PRICE * ABAS_ETH_PRICE

  el_safe('#USDValueof1ABAS').innerHTML = "<b> $ " + (ABAS_USD_PRICE).toLocaleString('en-US', {minimumFractionDigits: 0, maximumFractionDigits: 8}) + " </b>";
...
if(test5 != -1){
  ...
  auction_tot2 = getValueFromStats('Total Ethereum spent at Auctions', stats)
  
  el_safe('#TotalEthereumspentatAuctions').innerHTML = "<b>" + auction_tot2.toFixed(8) + " </b> ETH";
  ...
  circl = getValueFromStats('Tokens distributed via Mining', stats) * 2 + getValueFromStats('Current Auction', stats)*32768-32768
...
if(test51 != -1){
  ...
  el_safe('#TotalABASCirculatingSupply').innerHTML = "<b>" + circl2 + " </b> ABAS";
...
$.getJSON(urlz,
	...
	var timePerBlock = (1.0)*12*60*60 / diffBlock;
...
log("EFrom2 : ", epoch_From)
  total_Minted = getValueFromStats('Total ABAS Circulating Supply', stats)
...
//inflation math below
  inflation = (32768 / auction_Length * year_Time) + ((current_Reward * epoch_From) / seconds_Total ) * year_Time * 2
  inflation = inflation
log("auction_Length", 32768/auction_Length*year_Time) 
log("auction_LengthMinedTotal", (2 * (current_Reward * epoch_From) / seconds_Total ) * year_Time)
...
/* estimated hashrate */
 ...
 var tozz = ABAS_USD_PRICE * circl

  el_safe('#CirculatingSupplyMarketCap').innerHTML =  "<b> $ " + tozz.toLocaleString(undefined, {
  minimumFractionDigits: 0,
  maximumFractionDigits: 0
}) + "</b>";
...
function updateThirdPartyAPIs() {
  ...
  $.getJSON('https://raw.githubusercontent.com/Untouchable2k/Forge-Main/main/holdersandtrans3.html',
    function(data) {
...
function updateAllMinerInfo(eth, stats, hours_into_past){
  ...
  try {
    var last_diff_block_storage = Number(localStorage.getItem('lastDifficultyStartBlock_abas'));
    last_imported_mint_block = Number(localStorage.getItem('lastMintBlock_abas'));
    var mint_data = localStorage.getItem('mintData_abas');
...
console.log("DATAAMT: ", dataAmt);
    ...
    /* fill in block info */
    var dt = new Date();
    var innerhtml_buffer = '<tr><th>Time (Approx)</th><th>Arbitrum Block #</th>'
...
function getMinerInfoCSV(eth, stats, hours_into_past){
  ...
  .then((result) => {
    ...
    // Start file download.
    downloadTextAsFile("0xbtc-blocks_abas-" + date_now.toLocaleTimeString() + ".csv",
                       csv_text);
...
if(stat[0]=='Total Supply Staking Contract' || stat[0] =='Total Supply in all Sushiswap Pool'  || stat[0] =='RewardRate Pool 1'  || stat[0] =='RewardRate3 Pool 1'  || stat[0] =='Pool 1 Num1 Token'  || stat[0] =='Pool 1 Num2 Token'  || stat[0] =='Epoch Old'   || stat[0] =='SecondsPerDay Auction Contract' ){
	goodStat = false;
	if(!ShowStats){
		goodStat = true;
		}
...
if(stat_name =="u3"){
        ...
        if(areAllBlockchainStatsLoaded(stats)) {
          ...
          if(el('#minerstats')) {
            setTimeout(()=>{updateAllMinerInfo(eth, stats, 24)}, 1110);
          }
        }
      }
    }
